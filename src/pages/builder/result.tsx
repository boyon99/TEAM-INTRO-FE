...
function PerforManceAdd() {
    ...
    <form id='product'>
       <div className='mt-[48px]'>
        <span className='mt-[20px] text-GrayScalePrimary-700 font-bold text-[18px]/110%'>핵심성과 변경</span>
          ...
          <BuilderInput register={register('name')} title="성과지표 설명" type="text" placeholder="예: 핵심 지표" id="pageTitle" onChange={(e) => {
    ...
    <div className="text-GrayScalePrimary-600 font-[400] text-[12px] w-[256px] mt-[8px] pl-[2px]">
          ...
          <BuilderInput register={register('name')} title="성과지표 부가 설명" type="text" placeholder="예: 전년 대비 상승율" id="pageTitle" onChange={(e) => {
      const updatedProducts = products.map((product, index) => {
        if (index === products.length - 1) {
          return {
            ...product,
    ...
    <div className="text-GrayScalePrimary-600 font-[400] text-[12px] w-[256px] mt-[8px] pl-[2px]">
          ...
          <BuilderInput register={register('name')} title="숫자 지표" type="text" placeholder="예: 99%" id="pageTitle" onChange={(e) => {
      const updatedProducts = products.map((product, index) => {
        if (index === products.length - 1) {
          return {
            ...product,
...
export default function PerforMance() {
    ...
    return (
      <div>
        {add ? <Layout>{<PerforManceAdd />}</Layout> : <Layout>{<PerforManceView />}</Layout>}
